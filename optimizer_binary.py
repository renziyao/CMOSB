...
from secureboost.regressor import SecureBoostPlusRegressor
from datasets import (DefaultCredit, Synthetic, Credit)
...
save_dir_mlp = os.path.join(os.getcwd(), 'result/credit_1')
...
def generate_genotype_MLP(pop_size, tree_geno_size, depth_geno_size, max_lr):
    ...
    for _ in range(pop_size):
        ...
        Pt_sample_rate.append(np.random.uniform(0.1, 1.0))
        Pt_max_purity.append(np.random.uniform(0.7, 1.0))
...
class Optimize_Mnist_MLP():
    ...
    def cross_mutation(self, cross_over_rate, mutation_rate, n_c, n_m):
        ...
        Qt_lr = poly_mut(crossover_SBX(self.model_param['LEARNING_RATE'], n_c, cross_over_rate),
                         n_m, mutation_rate, self.max_lr, 0.01)
    ...
    def fitness(self, counter):
        ...
        for pop in range(pop_len):
            ...
            y_pred = fed.predict_prob(self.test_X)
    ...
    def add_constraint(self, fm):
        # for i in range(len(fm[0])):
        #     fm[2][i] = fm[2][i] + 20 * max(0, fm[2][i] - 0.7)
        # for i in range(len(fm[0])):
        #     fm[1][i] = fm[1][i] + 20 * max(0, fm[1][i] - 100)
        # for i in range(len(fm[0])):
        #     fm[0][i] = fm[0][i] + 20 * max(0, fm[0][i] - 0.1)
        return fm
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='synthetic1')
    ...
    if args.dataset == 'synthetic1':
        dataset = Synthetic(n_clients=2, n_samples=2000, n_clusters=3)
    elif args.dataset == 'defaultcredit':
        dataset = DefaultCredit(n_clients=2)
    else:
        ...
    genotype_set = {
         'pop_size': 20,

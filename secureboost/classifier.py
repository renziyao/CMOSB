...
class SecureBoostPlusClassifier():
    def __init__(
            self, 
            n_clients, 
            tree_num=30, 
    ...
    def calculate_histogram(self, I):
        ...
        for cid in range(self.n_clients):
            ...
            GH_vec = [[[0 for _ in range(self.class_num)] for _ in range(l_k + 1)] for _ in range(self.clients[cid].features.shape[1])]
            ...
            for i in range(self.clients[cid].features.shape[1]):
                for j in I:
                    for k in range(self.class_num):
                        GH_vec[i][self.clients[cid].features_bin[j][i]][k] += gh_vec[j][k]
    ...
    def subtract_hist(self, idx):
        ...
        GH = [[[[gh1[i][j][k][t]-gh2[i][j][k][t] for t in range(self.class_num)] for k in range(self.bin_num + 1)] for j in range(len(gh1[i]))] for i in range(self.n_clients)]
    ...
    def split_finding(self, ix, I, Fed=True):
        gh_vec = [sum([self.clients[0].raw_gh[i][j] for i in I]) for j in range(self.class_num)]
        g_vec, h_vec = [], []
        for gh in gh_vec:
        ...
        for i in range(self.n_clients):
            ...
            if Fed == False and i > 0:
                break
            GH_vec = copy.deepcopy(self.clients[0].GHs[ix][i])
            GH_num = copy.deepcopy(self.clients[0].GHnums[ix][i])
            cnt += len(GH_vec)
            for j in range(len(GH_vec)):
                for v in range(1, self.bin_num + 1):
                    for k in range(self.class_num):
                        GH_vec[j][v][k] += GH_vec[j][v-1][k]
                    GH_num[j][v] += GH_num[j][v-1]
                idx = 0
                while idx * batch_size < self.bin_num:
                    V_GH = [0 for _ in range(self.class_num)]
                    for t in range(min((idx + 1) * batch_size, self.bin_num), idx * batch_size, -1):
                        for k in range(self.class_num):
                            V_GH[k] = V_GH[k]*self.width**2 + GH_vec[j][t][k]
                    if i>0:
                        # if not V_GH==0: V_GH = self.private_key.decrypt(V_GH)
                        for t in range(self.class_num):
                            if self.enable_encrypt:
                                V_GH[t] = self.private_key.decrypt(V_GH[t])

                    for v in range(idx * batch_size + 1, min((idx + 1) * batch_size + 1, self.bin_num + 1)):
                        gh_vec = [v_gh % (self.width**2) for v_gh in V_GH]
                        g_l_vec, h_l_vec = [], []
                        for k in range(self.class_num):
                            V_GH[k] >>= 2*self.M
                            g_l, h_l = self.gh_unpack(gh_vec[k], GH_num[j][v])
                            g_l_vec.append(g_l)
                            h_l_vec.append(h_l)

                        g_r_vec = [g - g_l for (g, g_l) in zip(g_vec, g_l_vec)]
                        h_r_vec = [h - h_l for (h, h_l) in zip(h_vec, h_l_vec)]
                    
                        new_score = 0.5 * sum([g_l*g_l / (h_l + self.lamb) + \
                                        g_r*g_r / (h_r + self.lamb) - \
                                        g*g / (h + self.lamb) for (g_l, h_l, g_r, h_r, g, h) in zip(g_l_vec, h_l_vec, g_r_vec, h_r_vec, g_vec, h_vec)])
                        if new_score > score:
                            score = new_score
                            opt_val = {
                                "k_opt": j, 
        ...
        for opt_val in sorted(opt_val_client, key=lambda x: x['score'], reverse=True):
            ...
            for i in range(self.class_num):
                p.append(sum(sample == i) / len(sample))
            purity += weight * max(p)
    ...
    def calculate_gradient_and_hess(self):
        ...
        for i in self.clients[0].instance_sample:
            y_vec = self.clients[0].labels_onehot[i]
            p_vec = self.clients[0].y_pred[i]
            raw_gh_vec[i] = [
                self.gh_pack(p - y, 
                             p * (1 - p)
            ...
            if self.enable_encrypt:
                enc_gh_vec[i] = [self.public_key.encrypt(raw_gh) for raw_gh in raw_gh_vec[i]]
            else:
                enc_gh_vec[i] = copy.deepcopy(raw_gh_vec[i])
        self.clients[0].raw_gh = raw_gh_vec
        self.clients[0].enc_gh = enc_gh_vec
        t2 = time.process_time()
        if self.enable_encrypt:
    ...
    def get_samples(self):
        ...
        for i in range(len(self.clients[0].labels)):
            y = self.clients[0].labels_onehot[i]
            y_pred = self.clients[0].y_pred[i]
            # raw_g.append(math.exp(p)/(1+math.exp(p))-y)
            raw_g.append(sum([pp - yy for pp, yy in zip(y_pred, y)]))
    ...
    def calculate_leaf_purity(self, tree, sample_ids):
        ...
        for i in range(1, 1 << (self.height + 1)):
            ...
            for j in range(self.class_num):
                max_purity = max(max_purity, sum(sample == j) / len(sample))
            purity += weight * max_purity
            # print(weight, p0, p1)
    ...
    def calculate_privacy(self, tree, tree_fed, sample_ids):
        ...
        if self.all_sample == None:
            ...
            self.all_sample = []
            for i in range(self.class_num):
                cls_sample = list(np.where(self.clients[0].labels == i)[0])
                self.all_sample.extend(rnd.sample(cls_sample, min(n // self.class_num, len(cls_sample))))
            if len(self.all_sample) < n:
                self.all_sample.extend(rnd.sample(list(range(len(self.clients[0].labels))), n - len(self.all_sample)))
            self.mat = np.zeros((n, n), dtype=int)
        for i in range(1, 1 << (self.height + 1)):
            if sample_ids[i] == None:
                continue
            if tree[i] != None and tree_fed[i * 2] == True:
                continue
            if tree_fed[i] == False:
                continue
            # print('leaf: ', i)
    ...
    def build_tree(self):
        ...
        val[1] = [
            (
                sum([self.clients[0].labels_onehot[idx][i] for idx in sample_ids[1]]) - 
                sum([self.clients[0].y_pred[idx][i] for idx in sample_ids[1]])
            ) / len(sample_ids[1]) 
            for i in range(self.class_num)
        ]
        ...
        for layer in range(1, height+1):
            ...
            for i in range(1<<(layer-1), 1<<layer):
                ...
                if i * 2 + 1 < (1 << (height + 1)):
                    ...
                    val[i * 2] = [-g_l / (h_l + self.lamb) for (g_l, h_l) in zip(opt_val['g_l'], opt_val['h_l'])]
                    val[i * 2 + 1] = [-g_r / (h_r + self.lamb) for (g_r, h_r) in zip(opt_val['g_r'], opt_val['h_r'])]
                    if i * 2 >= (1 << height):
                        tree_fed[i * 2] = False
    ...
    def fit(self, features, labels):
        ...
        self.clients[0].labels_onehot = np.eye(self.class_num)[labels]
        self.clients[0].tree_roots = []
        y_tmp = np.zeros((labels.shape[0], self.class_num))
        self.clients[0].y_pred = np.zeros((labels.shape[0], self.class_num))
        for i in trange(self.tree_num):
            self.metrics['training_time'].append(0)
            for j in range(labels.shape[0]):
                for k in range(self.class_num):
                    self.clients[0].y_pred[j][k] = sigmoid(y_tmp[j][k])
    ...
    def predict_prob_single(self, features, tree):
        ypd = np.zeros((features[0].shape[0], self.class_num))
        for i in range(features[0].shape[0]):
            ...
            for j in range(self.class_num):
                ypd[i][j] += tree[0][pos][j] * self.lr
        return ypd

    def predict_prob(self, features):
        def sigmoid(x):
            return 1.0 / (1.0 + math.exp(-x))
        y_pred = np.zeros((features[0].shape[0], self.class_num))
